...
# 创建日志文件名（包含时间戳）
LOG_FILE="./output_1000epochs_bs128_houqi_sw/train_$(date +%Y%m%d_%H%M%S).log"

# 确保输出目录存在
mkdir -p ./output_1000epochs_bs128_houqi_sw/mbj_bandgap
...
nohup python train_with_cross_modal_attention.py \
    ...
    --output_dir ./output_1000epochs_bs128_houqi_sw \
    --num_workers 12 \
    --random_seed 42 \
    > "$LOG_FILE" 2>&1 &
