...
for exp_num in {1..4}; do
    ...
    for seed in "${seeds[@]}"; do
        log_file="$BASE_OUTPUT_DIR/exp${exp_num}_seed${seed}/nohup.log"
...
echo "    tail -f $BASE_OUTPUT_DIR/exp1_seed42/nohup.log"
echo ""
echo "  查看所有运行中的进程:"
if [ -f "$PID_FILE" ]; then
    mapfile -t PIDS < "$PID_FILE"
